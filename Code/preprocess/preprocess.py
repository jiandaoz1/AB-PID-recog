...
def resize_image(img):
    ...
    img_scale = float(2400) / float(img_size_min)
    print("scale factor = "+str(img_scale))
    if np.round(img_scale * img_size_max) > 3600:
        img_scale = float(2400) / float(img_size_max)
    ...
    for img_fn in img_fn_list:
        ...
        while True:
            ...
            if cv2.countNonZero(img)==0:
                break
        cv2.imwrite(os.path.join(output_path, os.path.basename(img_fn)), skel)
